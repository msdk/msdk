...
public class MSDKXICMethodTest {
  ...
  public void testXIC() throws MSDKException {
    ...
    // Execute the method
    MSDKXICMethod xicMethod = new MSDKXICMethod(rawFile, scans, mzRange, chromatogramType, store);
    ...
    Assert.assertEquals(new Integer(scans.size()), newChromatogram.getNumberOfDataPoints());
    ...
    for (int i = 0; i < newChromatogram.getNumberOfDataPoints(); i++) {
      ...
      Assert.assertEquals(rawFile.getScans().get(i).getChromatographyInfo(), rtValues[i]);
