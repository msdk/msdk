...
public class MSDKXICMethod implements MSDKMethod<Chromatogram> {
   ...
   */
  public MSDKXICMethod(@Nonnull RawDataFile rawDataFile, @Nonnull List<MsScan> scans,
      @Nonnull Range<Double> mzRange, @Nonnull ChromatogramType chromatogramType,
  ...
  public Chromatogram execute() throws MSDKException {
    ...
    logger.info("Started extracting XIC from file " + rawDataFile.getName());
    ...
    for (processedScans = 0; processedScans < totalScans; processedScans++) {
      ...
      if (chromRtValues[processedScans] == null)
        throw new MSDKException("Cannot extract chromatogram: scan #" + scan.getScanNumber()
            + " has no chromatography data");
      ...
      Integer topPeakIndex = MsSpectrumUtil.getBasePeakIndex(scanMzValues, scanIntensityValues,
          numOfDataPoints, mzRange);
      if (topPeakIndex != null)
        chromMzValues[processedScans] = scanMzValues[topPeakIndex];
      else
        chromMzValues[processedScans] = (mzRange.lowerEndpoint() + mzRange.upperEndpoint()) / 2.0;
      ...
      switch (chromatogramType) {
        ...
        case XIC:
        case SIC:
          chromIntensityValues[processedScans] =
              MsSpectrumUtil.getTIC(scanMzValues, scanIntensityValues, numOfDataPoints, mzRange);
          break;
        default:
          throw new MSDKException("Invalid chromatogram type: " + chromatogramType);
      }

    }

    // Create a new Chromatogram
    result = MSDKObjectBuilder.getChromatogram(store, 0, chromatogramType, SeparationType.UNKNOWN);
    result.setDataPoints(chromRtValues, chromMzValues, chromIntensityValues, totalScans);

    logger.info("Finished extracting XIC from file " + rawDataFile.getName());
    return result;
  }
