...
public class MeanFilterMethodTest {
    ...
    public void testMeanFilter() throws MSDKException {
        ...
        for (int i = 0; i < inputScans.size(); i++) {
            ...
            Assert.assertEquals(inputScan.getNumberOfDataPoints(),
                    newScan.getNumberOfDataPoints());

            for (int j = 0; j < inputScan.getNumberOfDataPoints(); j++) {
                Assert.assertEquals(mzValues[j], newMzValues[j], 0.00001);
                Assert.assertEquals(intensityValues[j], newIntensityValues[j],
                        0.00001);
            }
        ...
        for (int i = 0; i < inputScans.size(); i++) {
            ...
            float intensityAverage = inputScan.getTIC()
                    / inputScan.getNumberOfDataPoints();

            for (float intValue : intensityValues) {
                Assert.assertEquals(intensityAverage, intValue, 0.001);
            }
        }
    }
}
