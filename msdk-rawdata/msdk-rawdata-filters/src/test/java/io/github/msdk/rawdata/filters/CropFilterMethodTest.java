...
public class CropFilterMethodTest {
    ...
    public void testCropFilter() throws MSDKException {
        ...
        Range<Float> rtRange = Range.closed(
                scans.get(50).getChromatographyInfo().getRetentionTime(),
                scans.get(scans.size() - 30).getChromatographyInfo()
                        .getRetentionTime());
        Range<Double> scanRange = scans.get(0).getMzRange();
        Range<Double> mzRange = Range.closed(scanRange.lowerEndpoint() + 10,
                scanRange.upperEndpoint() - 10);
        DataPointStore store = DataPointStoreFactory.getMemoryDataStore();

        // Execute the filter
        CropFilterAlgorithm cropFilter = new CropFilterAlgorithm(mzRange,
                rtRange, store);
        ...
        for (MsScan newScan : newScans) {
            ...
            Assert.assertTrue(rtRange.contains(
                    newScan.getChromatographyInfo().getRetentionTime()));
