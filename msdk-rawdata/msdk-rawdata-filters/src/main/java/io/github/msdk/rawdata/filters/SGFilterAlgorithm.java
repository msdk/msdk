...
public class SGFilterAlgorithm implements MSDKFilteringAlgorithm {
    ...
    public MsScan performFilter(@Nonnull MsScan scan) {
        ...
        if (!Avalues.containsKey(sgDataPoints)
                || !Hvalues.containsKey(sgDataPoints)) {
            return scan;
        }
        ...
        for (int spectrumInd = marginSize; spectrumInd < (numOfDataPoints
                - marginSize); spectrumInd++) {

            // zero intensity data points must be left unchanged
            if (intensityBuffer[spectrumInd] == 0) {
                intensityBuffer[spectrumInd
                        - marginSize] = intensityBuffer[spectrumInd];
                continue;
            }
            ...
            for (int windowInd = 1; windowInd <= marginSize; windowInd++) {
                sumOfInts += aVals[windowInd]
                        * (intensityBuffer[spectrumInd + windowInd]
                                + intensityBuffer[spectrumInd - windowInd]);
            }
