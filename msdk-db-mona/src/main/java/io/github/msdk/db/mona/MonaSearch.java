...
public class MonaSearch implements MonaConfiguration {
   ...
   */
  public Iterator<MsSpectrum> findSimilarSpectra(long id, Integer minSimilarity)
      throws IOException {
   ...
   * @return
   */
  private @Nonnull Iterator<MsSpectrum> getMsSpectrumIterator(SimilaritySearchQuery query)
      throws IOException {
    // create a client and send data to the server
    final Client client = ClientBuilder.newBuilder().register(JacksonFeature.class)
        .register(new LoggingFilter()).build();
