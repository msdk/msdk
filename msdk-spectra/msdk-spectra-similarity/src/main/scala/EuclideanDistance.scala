...
/**
  * Created by wohlg_000 on 1/26/2016.
  */
class EuclideanDistance extends Similarity {

  /**
    * computes the Euclidean distance between two mass spectra, as defined here
    *
  ...
  def compute(unknown:Spectrum, library:Spectrum) : Double = {
    ...
    val norm = unknown.fragments.values.map(x => x.value * x.value).sum
    val diff = libraryIons.keySet.map(k => math.pow(libraryIons(k).value - unknownIons.getOrElse(k, zeroIntensity).value, 2)).sum
