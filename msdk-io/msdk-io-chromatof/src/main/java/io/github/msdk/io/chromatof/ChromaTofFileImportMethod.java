...
public class ChromaTofFileImportMethod implements MSDKMethod<FeatureTable> {
    ...
    private final Logger logger = LoggerFactory
            .getLogger(ChromaTofFileImportMethod.class);
    ...
    /**
     * <p>
     * Constructor for ChromaTofFileImportMethod. Uses {@link Locale#US} for
     * number parsing.
     * </p>
     *
     * @param sourceFile
    ...
    public FeatureTable execute() throws MSDKException {
        ...
        ChromaTofParser parser = new ChromaTofParser(fieldSeparator,
                quotationCharacter, Locale.US);
        LinkedHashSet<TableColumn> header = parser.parseHeader(sourceFile,
                normalizeColumnNames);
        int tcIndex = 0;
        for (TableColumn tc : header) {
            // Map the column name to the MSDK ColumnName
            FeatureTableColumn<?> column = createNewColumn(tc.getColumnName(),
                    fileSample);
        ...
        final List<TableRow> lines = parser.parseBody(header, sourceFile,
                normalizeColumnNames);
        ...
        for (TableRow tableRow : lines) {
            ...
            // Feature table row
            rowId++;
            FeatureTableRow row = MSDKObjectBuilder.getFeatureTableRow(
                    newFeatureTable, rowId);
