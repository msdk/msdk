...
public class ChromaTofFileImportMethod implements MSDKMethod<FeatureTable> {
    ...
    public FeatureTable execute() throws MSDKException {
        ...
        ChromaTofParser parser = new ChromaTofParser(ChromaTofParser.FIELD_SEPARATOR_COMMA, ChromaTofParser.QUOTATION_CHARACTER_DOUBLETICK, Locale.US);
        ...
        try {
            lines = Files.asCharSource(sourceFile, Charset.defaultCharset())
                    .readLines();
        ...
        for (String line : lines) {
            ...
            if (firstLine) {
                ...
                for (int i = 0; i < columnNames.size(); i++) {
                    ...
                    FeatureTableColumn<?> column = createNewColumn(columnName,
                            sample);
                    ...
                    if (column.getName()
                            .equals(ColumnName.IONANNOTATION.getName())) {
                ...
                // Feature table row
                rowId++;
                FeatureTableRow row = MSDKObjectBuilder
                        .getFeatureTableRow(newFeatureTable, rowId);
                    ...
                    if (currentClass.getSimpleName()
                        ...
                        case "molecular formula":
                            ...
                            IMolecularFormula formula = MolecularFormulaManipulator
                                    .getMolecularFormula(stringData,
    ...
    private FeatureTableColumn<?> createNewColumn(String columnName,
            Sample sample) {
        ...
        for (ColumnName defaultName : ColumnName.values()) {
            if (columnName.toUpperCase()
                    .contains(defaultName.getName().toUpperCase()))
    ...
    private void findNames(String firstLine) {
        ...
        for (int firstIndex = columns.length
                - 1; firstIndex >= 0; firstIndex--) {
            ...
            for (int characters = 0; characters <= firstColumnLength; characters++) {
                ...
                for (int secondIndex = firstIndex
                        - 1; secondIndex >= 0; secondIndex--) {
                    String column = columns[secondIndex];
                    int columnLength = column.length();

                    if (columnLength >= currentCharacterLength) {
                        String firstName = firstColumn.substring(characters,
                                firstColumnLength);
                        String secondName = column.substring(
                                columnLength - currentCharacterLength,
                                columnLength);
            ...
            if (maxCharacters > 2) {
                commonName = firstColumn.substring(
                        firstColumnLength - maxCharacters, firstColumnLength);
                sampleName = firstColumn.substring(0,
                        firstColumnLength - maxCharacters);
        ...
        if (samples.size() == 0) {
            ...
            for (int firstIndex = columns.length
                ...
                // right to left
                int maxCharacters = 0;
                for (int characters = firstColumnLength
                        - 1; characters >= 0; characters--) {
                    int currentCharacterLength = firstColumnLength - characters;
                    ...
                    if (nameCounter > 2 && !colums.contains(commonName)) {
                        ...
                        for (int i = 0; i < columns.length; i++) {
                            ...
                            if (columnName.contains(commonName)) {
                                String sampleName = columnName
                                        .replace(commonName, "");

                                // Replace ending space if present
                                if (sampleName.endsWith(" "))
                                    sampleName = sampleName.substring(0,
                                            sampleName.length() - 1);
