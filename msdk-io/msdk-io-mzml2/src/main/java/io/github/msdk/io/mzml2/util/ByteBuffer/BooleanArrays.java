 ...
 *
 * <p>
 * In particular, the <code>ensureCapacity()</code>, <code>grow()</code>, <code>trim()</code> and
 * <code>setLength()</code> methods allow to handle arrays much like array lists. This can be very
...
public class BooleanArrays {
   ...
   *
   * <P>
   * If you cannot foresee whether this array will need again to be enlarged, you should probably
   * use <code>grow()</code> instead.
   *
   * @param array an array.
   * @param length the new minimum length for this array.
   * @return <code>array</code>, if it contains <code>length</code> entries or more; otherwise, an
   *         array with <code>length</code> entries whose first <code>array.length</code> entries
   *         are the same as those of <code>array</code>.
   */
   ...
   *         an array with <code>length</code> entries whose first <code>preserve</code> entries are
  ...
  /**
   * Grows the given array to the maximum between the given length and the current length multiplied
   * by two, provided that the given length is larger than the current length.
   *
   * <P>
   ...
   * @param array an array.
   * @param length the new minimum length for this array.
   * @return <code>array</code>, if it can contain <code>length</code> entries; otherwise, an array
   *         with max(<code>length</code>,<code>array.length</code>/&phi;) entries whose first
  ...
  public static boolean[] grow(final boolean[] array, final int length) {
    if (length > array.length) {
      final int newLength =
          (int) Math.max(Math.min(2L * array.length, Arrays.MAX_ARRAY_SIZE), length);
  ...
  /**
   * Grows the given array to the maximum between the given length and the current length multiplied
   * by two, provided that the given length is larger than the current length, preserving just a
   * part of the array.
   *
   * <P>
   * If you want complete control on the array growth, you should probably use
   ...
   * @return <code>array</code>, if it contains <code>length</code> entries or less; otherwise, an
   *         array with <code>length</code> entries whose entries are the same as the first
   ...
   * @return <code>array</code>, if it contains exactly <code>length</code> entries; otherwise, if
   *         it contains <em>more</em> than <code>length</code> entries, an array with
   *         <code>length</code> entries whose entries are the same as the first <code>length</code>
   *         entries of <code>array</code>; otherwise, an array with <code>length</code> entries
   ...
   *         <code>offset</code>+<code>length</code> is greater than the array length.
   */
   ...
   * used to sort primitive types in {@link java.util.Arrays}, which switches to mergesort on large
   * inputs.
   * 
   ...
   * @param from the index of the first element (inclusive) to be sorted.
   * @param to the index of the last element (exclusive) to be sorted.
   * @param supp a support array containing at least <code>to</code> elements, and whose entries are
   *        identical to those of {@code a} in the specified range.
   */
  @SuppressWarnings("unchecked")
  ...
  @SuppressWarnings("unchecked")
  public static void mergeSort(final boolean a[], final int from, final int to,
      BooleanComparator comp, final boolean supp[]) {
    int len = to - from;
    // Insertion sort on smallest arrays
    if (len < SMALL) {
   ...
   * @return <code>a</code>.
   */
  public static boolean[] shuffle(final boolean[] a, final int from, final int to,
      final Random random) {
  ...
  private static final class ArrayHashStrategy
      implements Hash.Strategy<boolean[]>, java.io.Serializable {
   ...
   *
   * <P>
   * This hash strategy may be used in custom hash collections whenever keys are arrays, and they
   * must be considered equal by content. This strategy will handle <code>null</code> correctly, and
   * it is serializable.
   */
