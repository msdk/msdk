...
import io.github.msdk.io.mzml2.util.MSNumpress;
...
public class MzMLPeaksDecoder {
	...
	/**
	 * Converts a base64 encoded mz or intensity string used in mzML files to an
	 * array of doubles. If the original precision was 32 bit, you still get
	 * doubles as output, would be too complicated to provide another method to
	 * parseIndexEntries them as floats. Hopefully some day everything will be
	 * in 64 bits anyway.
	 * 
	 * @param bytesIn
	 *            Byte array, decoded from a base64 encoded string<br/>
	 *            E.g. like: eNoNxltIkwEYBuAOREZFhrCudGFbbraTU+Zmue...
	 * @param lengthIn
	 *            length of data to be treated as values, i.e. the input array
	 *            can be longer, the values to be interpreted must start at
	 *            offset 0, and this will indicate the length
	 * @param precision
	 *            allowed values: 32 and 64, can be null only if MS-NUMPRESS
	 *            compression was applied and is specified in the @{code
	 *            compressions} enum set.
	 * @param numPoints
	 * @param compressions
	 *            null or {@link MzMLBinaryDataInfo.MzMLCompressionType#NONE}
	 *            have the same effect. Otherwise the binary data will be
	 *            inflated according to the compression rules.
	 * @return
			...
			case (64): {
				...
				for (int i = 0; i < numPoints; i++) {
					...
					asLong = ((long) (decoded[offset] & 0xFF)) // zero shift
							| ((long) (decoded[offset + 1] & 0xFF) << 8) | ((long) (decoded[offset + 2] & 0xFF) << 16)
							| ((long) (decoded[offset + 3] & 0xFF) << 24) | ((long) (decoded[offset + 4] & 0xFF) << 32)
							| ((long) (decoded[offset + 5] & 0xFF) << 40) | ((long) (decoded[offset + 6] & 0xFF) << 48)
