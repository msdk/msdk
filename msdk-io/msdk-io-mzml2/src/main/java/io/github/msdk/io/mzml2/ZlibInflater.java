...
public class ZlibInflater {
	...
	/**
	 * The ByteArrayHolder pool, that this decoder is using. If you use
	 * uncompression methods from this class, e.g.
	 * {@link #zlibUncompressBuffer(byte[], int, Integer)}, you will need to
	 * return the ByteArrayHolder, that was returned to you, into that pool.
	 * 
	 * @return
	 */
	 ...
	 * {@link #zlibUncompressBuffer(byte[], int, Integer)}.<br/>
	 * Inflates zLib compressed byte[].
	 * 
	 * @param compressed
	 *            zLib compressed bytes
	 * @param uncompressedLen
	 *            length of data in bytes when uncompressed. Optional.
	 * @return inflated byte array, which is borrowed from pool
	 *         ({@link #getPool() }). You MUST return the byte holder back to
	 *         the pool after usage.
	 * @throws IOException
	 *             should never happen, ByteArrayOutputStream is in-memory
	 * @throws DataFormatException
	 *             in case of malformed input byte array
	 ...
	 * 
	 * @param bytes
	 *            zLib compressed bytes in a holder, with properly set position
	 * @param length
	 *            length of data in the input array to be used
	 * @param uncompressedLen
	 *            length of data in bytes when uncompressed. Optional.
	 * @return inflated byte array, which is borrowed from pool
	 *         ({@link #getPool() }). You MUST return the byte holder back to
	 ...
	 * @throws MSDKException
	 */
	public static ByteArrayHolder zlibUncompressBuffer(byte[] bytes, int length, Integer uncompressedLen)
			throws IOException, DataFormatException, MSDKException {
