...
public class MzTabFileImportMethod implements MSDKMethod<FeatureTable> {
  ...
  private Map<String, FeatureTableColumn<?>> tableColumns =
      new HashMap<String, FeatureTableColumn<?>>();
  ...
  private void addColumns(@Nonnull FeatureTable featureTable, @Nonnull MZTabFile mzTabFile) {
    ...
    for (Entry<Integer, MsRun> entry : msrun.entrySet()) {
      ...
      column = MSDKObjectBuilder.getFeatureTableColumn(ColumnName.AREA, sample);
      newFeatureTable.addColumn(column);
      tableColumns.put("[" + entry.getKey() + "]_" + ColumnName.AREA.getName(), column);

      // Optional columns
      String mzTabColumnName;
      SortedMap<String, MZTabColumn> columnsMap =
          mzTabFile.getSmallMoleculeColumnFactory().getOptionalColumnMapping();
      for (Entry<String, MZTabColumn> entry2 : columnsMap.entrySet()) {
        mzTabColumnName = entry2.getValue().getName();
        column = getFeatureTableColumn(mzTabColumnName, sample, msRunKey);
    ...
    for (SmallMolecule smallMolecule : smallMolecules) {
      ...
      for (Entry<Integer, Assay> entry : assayMap.entrySet()) {
        ...
        if (smallMolecule.getAbundanceColumnValue(sampleAssay) != null) {
          featureArea =
              Double.parseDouble(smallMolecule.getAbundanceColumnValue(sampleAssay).toString());
  ...
  @Nullable
  private FeatureTableColumn<?> getFeatureTableColumn(@Nonnull String mzTabColumnName,
      @Nonnull Sample sample, int msRunKey) {
    if (mzTabColumnName.contains("[" + msRunKey + "]_")) {
      ...
      if (columnName == null) {
        return MSDKObjectBuilder.getFeatureTableColumn(name, String.class, sample);
      } else {
        return MSDKObjectBuilder.getFeatureTableColumn(columnName, sample);
      }
    } else {
      return null;
    }
  ...
  private ColumnName mzTabNameToColumnName(String mzTabColumnName) {
    ...
    for (ColumnName columnName : ColumnName.values()) {
      if (compareName.toUpperCase().equals(columnName.toString().toUpperCase()))
        return columnName;
    }
    return null;
  }
