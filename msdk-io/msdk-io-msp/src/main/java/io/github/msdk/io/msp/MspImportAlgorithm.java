...
public class MspImportAlgorithm {
    ...
    private static final Pattern numPeaksPattern = Pattern
            .compile("^Num Peaks: (\\d+)");
    ...
    public static @Nonnull MspSpectrum parseMspFromString(
        ...
        while (scanner.hasNextLine()) {
            ...
            while (m.find()) {
                ...
                intensityValues = ArrayUtil.addToArray(intensityValues,
                        intensity, size);
                size++;
            }
        ...
        if (numPeaks == null)
            throw new MSDKException(
                    "Invalid format of MSP file, could not find Num Peaks: entry");
