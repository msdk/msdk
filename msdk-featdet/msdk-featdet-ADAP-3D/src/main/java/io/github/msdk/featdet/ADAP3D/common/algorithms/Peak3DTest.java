...
public class Peak3DTest {
	...
	/**
	 * <p>
	 * execute method is used for testing a peak with given m/z-value (variable mz)
     * and left and right bounds (variables leftBound and rightBound).
     *
     * Peak is tested by comparing similarity between adjacent m/z slices.
     *
     * Let mzValues be a sorted list of all m/z values in the profile data. Let index be an integer such that
     *
     *    mzValues[index] == mz
     *
     * We find similarities between the EIC corresponding to m/z value mzValues[index] and adjacent EICs
     * corresponding to
     *
     *    ..., mzValues[index-2], mzValues[index-1], mzValues[index+1], mzValues[index+2], ...
     *
     * as long as those similarities are higher than the similarity threshold. First, we check each m/z-value higher
     * than mzValues[index], stop when the current similarity becomes lower than the similarity threshold, and save the last
     * m/z-value (variable upperMZbound). Next, we check each m/z-value lower than mzValues[index], stop when the
     * current similarity becomes lower than the similarity threshold, and save the last m/z-value (variable lowerMZbound)
     *
     * Peak is considered to be good if the differences
     *    upperMZbound - mzValues[index],
     *    mzValues[index] - lowerMZbound,
     *    upperMZbound - lowerMZbound
	...
	public Result execute(double mz, int leftBound, int rightBound){
		...
		//normalize method is used for normalizing intensities for given mz value. It updates referenceEIC.
		...
		int upperMzBound = findMZbound(leftBound,rightBound,roundedMz,
				roundedFWHM,mzIndex,referenceEIC,similarityValues, Direction.UP);
		
		//Here we're getting lowest mz value for which the EIC is similar to given mz value.
		int lowerMzBound = findMZbound(leftBound,rightBound,roundedMz,
				roundedFWHM,mzIndex,referenceEIC,similarityValues, Direction.DOWN);
	...
	/**
	 * <p>
	 * findMZbound method is used to compare adjacent EICs to the reference EIC and return the last m/z value such that
     * its corresponding EIC is similar to the reference EIC.
     *
     * The similarity is calculated between two EICs in three steps:
     * 1. Normalize EIC
     * 2. Find area of the difference of the normalized EICs (variable diffArea)
     * 3. Calculate similarity by the formula
     *
	...
	private int findMZbound(int leftBound, int rightBound, int roundedMz,
                            double roundedFWHM, int mzIndex, double[] referenceEIC , List<Double> similarityValues,
                            Direction direction){
...
//			double originalCurMZ = (double) curMZ / objsliceSparseMatrix.roundMzFactor;
			...
			for(int j=0;j<arrayCount-1;j++){
				diffArea += Math.abs(0.5*((referenceEIC[j] - curEIC[j])+(referenceEIC[j+1] - curEIC[j+1])));
			}
	...
	private double normalize(MultiKeyMap slice, int leftBound, int rightBound, int roundedMz, double[] referenceEIC){
		...
		for(int i=0;i<arrayCount;i++){
		    SliceSparseMatrix.Triplet triplet = (SliceSparseMatrix.Triplet)slice.get(i+leftBound, roundedMz);
